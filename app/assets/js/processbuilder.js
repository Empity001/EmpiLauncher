...
class ProcessBuilder {
    ...
    build(){
        ...
        // Hide credentials and the temporary EmpiCheck session marker.
        ...
        for(let i = 0; i < loggableArgs.length; i++){
            if(typeof loggableArgs[i] === 'string' && loggableArgs[i].startsWith('-Dempi.launcher.session=')){
                loggableArgs[i] = '-Dempi.launcher.session=**********'
            }
        }
        ...
        // Printing every Minecraft line into Chromium's console is extremely
        // expensive for large modpacks. Keep it for npm start / explicit
        // diagnostics, and write one chunk instead of one console call per line.
        const mirrorMinecraftOutput = isDev || process.env.EMPILAUNCHER_MC_CONSOLE === '1'
        if(mirrorMinecraftOutput){
            child.stdout.on('data', data => {
                const output = data.trimEnd()
                if(output){
                    console.log(`\x1b[32m[Minecraft]\x1b[0m ${output}`)
                }
            })
            child.stderr.on('data', data => {
                const output = data.trimEnd()
                if(output){
                    console.log(`\x1b[31m[Minecraft]\x1b[0m ${output}`)
                }
            })
        }
        child.on('close', (code) => {
     ...
     */
    /**
     * Build the temporary JVM properties consumed by EmpiCheck Client.
     * These properties exist only inside the Minecraft process launched here.
     */
    _constructEmpiCheckArguments(){
        ...
        const rawServer = this.server?.rawServer || {}

        return [
            '-Dempi.launcher=true',
            `-Dempi.launcher.name=${clean('Empi Launcher')}`,
            `-Dempi.launcher.version=${clean(this.launcherVersion)}`,
            `-Dempi.launcher.session=${clean(this.empiLaunchSession)}`,
            `-Dempi.pack.id=${clean(rawServer.id)}`,
            `-Dempi.pack.name=${clean(rawServer.name, rawServer.id)}`,
            `-Dempi.pack.version=${clean(rawServer.version)}`
        ]
    }
    ...
    _constructJVMArguments112(mods, tempNativePath){
        ...
        if(process.platform === 'darwin'){
            args.push('-Xdock:name=Empi Launcher')
                ...
                if(argDiscovery.test(args[i])){
                    ...
                    switch(identifier){
                        ...
                        case 'natives_directory':
                            ...
                            break
                        case 'launcher_name':
                            val = args[i].replace(argDiscovery, 'Empi-Launcher')
                            break
