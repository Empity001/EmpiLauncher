 ...
 * fades in.
 */
function switchView(current, next, currentFadeTime = 140, nextFadeTime = 140, onCurrentFade = () => {}, onNextFade = () => {}){
    currentView = next

    // Keep explicit long transitions from making the whole launcher feel delayed.
    const fadeOutTime = Math.min(Number(currentFadeTime) || 0, 160)
    const fadeInTime = Math.min(Number(nextFadeTime) || 0, 160)

    $(`${current}`).stop(true, true).fadeOut(fadeOutTime, async () => {
        await onCurrentFade()
        $(`${next}`).stop(true, true).fadeIn(fadeInTime, async () => {
            await onNextFade()
        })
    })
}
...
async function showMainUI(data){
    ...
    setTimeout(() => {
        ...
        // landing.js clears the startup fallback only after the selected
        // background has been decoded. Removing it here caused a black gap.
        $('#main').show()
        ...
        if(ConfigManager.isFirstLaunch()){  
            ...
            $(VIEWS.welcome).fadeIn(220)
        } else {
            if(isLoggedIn){
                currentView = VIEWS.landing
                $(VIEWS.landing).fadeIn(220)
            } else {
                loginOptionsCancelEnabled(false)
                loginOptionsViewOnLoginSuccess = VIEWS.landing
                loginOptionsViewOnLoginCancel = VIEWS.loginOptions
                currentView = VIEWS.loginOptions
                $(VIEWS.loginOptions).fadeIn(220)
            }
        }

        setTimeout(() => {
            $('#loadingContainer').fadeOut(180, () => {
