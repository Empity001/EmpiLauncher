...
const dataPath = path.join(sysRoot, '.EmpiLauncher')
...
exports.getDataDirectory = function(def = false){
    ...
    return isValidDataDirectory(configuredDirectory) ? configuredDirectory : dataPath
}
...
exports.setDataDirectory = function(dataDirectory){
    ...
    if(config.settings.launcher == null || typeof config.settings.launcher !== 'object'){
        config.settings.launcher = {}
    }

    config.settings.launcher.dataDirectory = isValidDataDirectory(dataDirectory)
        ? dataDirectory.trim()
        : dataPath
}
...
exports.getAbsoluteMinRAM = function(ram){
    // NOTA: el pack fuerza un mínimo de ram.minimum (normalmente 3GB) vía
    // distribution.json remoto. Lo ignoramos localmente para poder bajar el
    // slider en equipos con poca RAM total. Cambiá MIN_RAM_FLOOR_GB si querés
    // otro piso.
    const MIN_RAM_FLOOR_GB = 0.5
    return MIN_RAM_FLOOR_GB
}
...
function isValidDataDirectory(value){
...
function repairInvalidDataDirectory(){
    if(config?.settings == null || typeof config.settings !== 'object'){
        config.settings = {}
    }
    if(config.settings.launcher == null || typeof config.settings.launcher !== 'object'){
    ...
    const configuredDirectory = config.settings.launcher.dataDirectory
    if(isValidDataDirectory(configuredDirectory)){
        // Remove accidental whitespace without touching valid custom locations.
        config.settings.launcher.dataDirectory = configuredDirectory.trim()
        return false
    }

    logger.warn(`Invalid launcher data directory (${String(configuredDirectory)}). Restoring ${dataPath}.`)
    config.settings.launcher.dataDirectory = dataPath
    return true
}

function ensureConfiguredDataDirectory(){
    const configuredDirectory = exports.getDataDirectory()
    try {
        fs.ensureDirSync(configuredDirectory)
    } catch(err) {
        logger.error(`Unable to create the configured data directory: ${configuredDirectory}`, err)

        // An invalid or inaccessible default must never leave the launcher showing null.
        // Preserve valid custom paths; only fall back when the configured value itself is invalid.
        if(!isValidDataDirectory(config?.settings?.launcher?.dataDirectory)){
            config.settings.launcher.dataDirectory = dataPath
...
function migrateLegacyDataDirectory(){
    const configuredDirectory = config?.settings?.launcher?.dataDirectory
    const usingLegacyDirectory = samePath(configuredDirectory, legacyDataPath)
    const canAdoptLegacyDirectory = samePath(configuredDirectory, dataPath)
        && fs.existsSync(legacyDataPath)
        && !fs.existsSync(dataPath)
    if(!usingLegacyDirectory && !canAdoptLegacyDirectory){
        return false
    }
    ...
    try {
        if(fs.existsSync(legacyDataPath)){
            if(!fs.existsSync(dataPath)){
                logger.info(`Migrating launcher data from ${legacyDataPath} to ${dataPath}.`)
                fs.moveSync(legacyDataPath, dataPath)
            } else {
                logger.info(`Merging legacy launcher data from ${legacyDataPath} into ${dataPath}.`)
                fs.copySync(legacyDataPath, dataPath, {
                    overwrite: false,
                    errorOnExist: false
                })
                logger.warn(`Legacy data was preserved at ${legacyDataPath} as a safety backup.`)
            }
        } else {
            fs.ensureDirSync(dataPath)
        }
        ...
        logger.error('Unable to migrate legacy launcher data. The old directory will continue to be used.', err)
        return false
    }
}
...
exports.load = function(){
    ...
    if(!fs.existsSync(configPath)){
        ...
        if(fs.existsSync(configPathLEGACY)){
            ...
            repairInvalidDataDirectory()
            migrateLegacyDataDirectory()
            ensureConfiguredDataDirectory()
            exports.save()
        }
    }
    if(doLoad){
    ...
    logger.info(`Successfully Loaded. Data directory: ${config.settings.launcher.dataDirectory}`)
}
...
exports.getLanguage = function(def = false){
    ...
    const language = config?.settings?.launcher?.language
    return ['auto', 'en_US', 'es_ES'].includes(language) ? language : fallback
}
...
exports.setLanguage = function(language){
    config.settings.launcher.language = ['auto', 'en_US', 'es_ES'].includes(language)
        ? language
        : DEFAULT_CONFIG.settings.launcher.language
}

/**
 * Modo de optimización absoluta (sin fondos, banners ni animaciones).
 *
 * 'auto' sigue la detección automática por RAM (menos de 6 GB la activa).
 * 'on'/'off' fuerzan el estado manualmente, sin importar la RAM del equipo.
 *
 * @param {boolean} def Optional. If true, the default value will be returned.
 * @returns {'auto'|'on'|'off'}
 */
exports.getPerformanceMode = function(def = false){
    const fallback = DEFAULT_CONFIG.settings.launcher.performanceMode
    if(def){
        return fallback
    }

    const mode = config?.settings?.launcher?.performanceMode
    return ['auto', 'on', 'off'].includes(mode) ? mode : fallback
}

/**
 * Guarda el modo de optimización absoluta seleccionado manualmente.
 *
 * @param {'auto'|'on'|'off'} performanceMode
 */
exports.setPerformanceMode = function(performanceMode){
    config.settings.launcher.performanceMode = ['auto', 'on', 'off'].includes(performanceMode)
        ? performanceMode
        : DEFAULT_CONFIG.settings.launcher.performanceMode
}